...
const McCutchenCollection = props => {
  const [mcCutchenDataIsLoading, setMcCutchenDataIsLoading] = useState(false)
  const [inventoryMcCutchenData, setInventoryMcCutchenData] = useState([])

  const fetchData = useCallback(async () => {
    setMcCutchenDataIsLoading(true)
    axios
      .get(
        `https://api.showzone.io/api/user-inventory/${props.currentUser?.uid}/legends-collection-mccutchen/`
      )
      .then(async getUserInventoryLegendData => {
        if (getUserInventoryLegendData.data) {
          setInventoryMcCutchenData(getUserInventoryLegendData.data.results[0])
          setMcCutchenDataIsLoading(false)
        } else {
          setMcCutchenDataIsLoading(false)
        }
      })
              ...
              >
                {props.inventoryExists ? (
                  ...
                  >
                    <Doughnut
                      data={{
                        ...
                        datasets: [
                          {
                            data: [
                              inventoryMcCutchenData?.legends_total
                                ?.cards_owned,
                              inventoryMcCutchenData?.legends_total
                                ?.cards_required -
                                inventoryMcCutchenData?.legends_total
                    ...
                    >
                      {Math.round(
                        (inventoryMcCutchenData?.legends_total?.cards_owned /
                          inventoryMcCutchenData?.legends_total
                            ?.cards_required) *
                          100
                      )}
                ...
                {props.inventoryExists ? (
                  ...
                  >
                    ...
                    >
                      {Math.round(
                        ((inventoryMcCutchenData?.legends_total?.total_cost -
                          inventoryMcCutchenData?.legends_total?.stubs_needed) /
                          inventoryMcCutchenData?.legends_total?.total_cost) *
                          100
                      )}
                      %
...
export default McCutchenCollection
