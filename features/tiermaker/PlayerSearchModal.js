...
const PlayerSearchModal = ({
  ...
  let [highlightedPlayer, setHighlightedPlayer] = useState(-1)
  ...
  const addPlayerForComparison = useCallback(
    player => {
        assignCardToSlot(player)
    },
    [router, assignCardToSlot]
  )
  ...
  const fetchPlayersForComparison = useCallback(async () => {
    try {
      ...
      const searchPlayersResponse = await axios.get(
        `https://api.showzone.io/api/player-profiles/?format=json&game=${gamesToInclude}&order_by=desc%20playerprofileadvanced__overall_true&exclude_card_id=${playersToExclude}${urlParams.toString()}`
      )
  ...
  useEffect(() => {
    document.addEventListener('keydown', (event) => {
        ...
        if (event.key === 'ArrowDown') {
            ...
            highlightedPlayer = highlightedPlayer + 1
        ...
        if(event.key === 'Enter') {
            if(openModal) {
              if(playerSearchData[highlightedPlayer]) {
                handleCloseModal()
                window.location.replace(`/players/${playerSearchData[highlightedPlayer]?.card_id}`)
            }
          }
        }        
    });
      ...
      <ModalPaper>
        ...
        /> */}
          {playerSearchData.map(player => (
            <PlayerListing
              key={player.card_id}
              sx={{ background: (playerSearchData[highlightedPlayer]?.card_id === player.card_id ? '#ed2024' : 'transparent') }}
